let entries = JSON.parse(localStorage.getItem('games')) || [];

function saveEntries() {
    localStorage.setItem('games', JSON.stringify(entries));
}

function renderEntries() {
    const list = document.getElementById('entry-list');
    if (entries.length === 0) {
        list.innerHTML = '<p class="empty-msg">No games added yet. Add one above!</p>';
        return;
    }
    list.innerHTML = entries.map((entry, index) => `
        <div class="entry-card games">
