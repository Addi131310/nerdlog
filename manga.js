let entries = JSON.parse(localStorage.getItem('manga')) || [];

function saveEntries() {
    localStorage.setItem('manga', JSON.stringify(entries));
}

function renderEntries() {
    const list = document.getElementById('entry-list');
    if (entries.length === 0) {
        list.innerHTML = '<p class="empty-msg">No manga added yet. Add one above!</p>';
        return;
    }
    list.innerHTML = entries.map((entry, index) => `
        <div class="entry-card manga">
